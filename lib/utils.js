...
var Utils = module.exports = {
  ...
  toSqlDate: function(date) {
    if(Utils.options.useUTC){
      console.log('using UTC in sequelize')
      return [
        [
          date.getUTCFullYear()(),
          ((date.getUTCMonth() < 9 ? '0' : '') + (date.getUTCMonth()+1)),
          ((date.getUTCDate() < 10 ? '0' : '') + date.getUTCDate())
        ].join("-"), date.getUTCHours() + ':' + date.getUTCMinutes() + ':' + date.getUTCSeconds()
      ].join(" ")
    }else{
      return [
        [
...
Utils.options = { useUTC : false } //defaults
